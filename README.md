Вот полностью переписанный класс с учётом требований кейса:
— возвращает **JSON с drafts**
— поддерживает **несколько объявлений при `shouldSplit = true`**
— содержит fallback
— убран старый `generate_structured`
...
✔ Соответствие формату кейса
✔ Несколько `drafts`, когда нужно
✔ Устойчивость к кривому JSON от модели
✔ Готово к отправке в решение

---

Если хочешь следующий шаг — могу помочь сделать **автоматическое определение mcId из словаря (это сильно повышает баллы в кейсе)**.
